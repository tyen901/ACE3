...
// Function handles both weapon attachment panels (flat tree) and container panels (grouped tree)
// Weapon panels modify loadout directly, container panels only show item info
private _item = _control tvData _selectionPath;

// For container panels, show item info and refresh availability - don't modify loadout
if (GVAR(currentLeftPanel) in [IDC_buttonUniform, IDC_buttonVest, IDC_buttonBackpack]) then {
    ...
    [_display, _control, _selectionPath, _cfgEntry] call FUNC(itemInfo);
} else {
    // Weapon attachment panel - use original logic
